...
function AddEmoteMenu(menu)
    ...
    if not Config.SqlKeybinding then
        ...
        keyinfo = NativeUI.CreateItem(Config.Languages[lang]['keybinds'],
            Config.Languages[lang]['keybindsinfo'] .. " /emotebind [num4-9~w~] [emotename~w~]")
    ...
    if Config.SharedEmotesEnabled then
        for a, b in pairsByKeys(RP.Shared) do
            ...
            if otheremotename == nil then
                shareitem = NativeUI.CreateItem(z, "/nearby (" .. a .. "~w~)")
            else
                shareitem = NativeUI.CreateItem(z,
                    "/nearby (" ..
                    a .. "~w~) " .. Config.Languages[lang]['makenearby'] .. " (" .. otheremotename .. "~w~)")
            end
            ...
            if #results > 0 then
                ...
                for k, v in pairs(results) do
                    ...
                    if v.table == "Shared" then
                        ...
                        if otheremotename == nil then
                           desc = "/nearby (" .. v.name .. "~w~)"
                        else
                           desc = "/nearby (" .. v.name .. "~w~) " .. Config.Languages[lang]['makenearby'] .. " (" .. otheremotename .. "~w~)"
                        end
                    else
...
--         infomenu = _menuPool:AddSubMenu(menu, Config.Languages[lang]['infoupdate'], "~h~The RPEmotes Team & Collaborators~h~", "",
--             Menuthing, Menuthing)
--     else
...
--     infomenu:AddItem(NativeUI.CreateItem("Thanks ~o~DullPear 🍐", "~o~DullPear for the original dpemotes ❤️"))
--     infomenu:AddItem(NativeUI.CreateItem("Thanks <b>Kibook 🐩</b>",
--         "<b>Kibook</b> for the addition of Animal Emotes 🐩 submenu."))
--     infomenu:AddItem(NativeUI.CreateItem("Thanks AvaN0x 🇫🇷",
--         "AvaN0x 🇫🇷 for reformatting and assisting with code and additional features 🙏"))
...
--     infomenu:AddItem(NativeUI.CreateItem("Thanks Ultrahacx 🧑‍💻",
-- 	"Ultrahacx for the custom emotes ☺️."))
